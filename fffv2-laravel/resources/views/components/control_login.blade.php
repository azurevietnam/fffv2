<div class="form-group {{ $group_class }} {{ $columns == 0 ? '': 'col-lg-' . $columns }} {{ $errors->has($name) ? 'has-error' : '' }}">
    <div class="col-xs-12">
        @if($label)
        ...
        @else
            @if($required == 1)
                {!! Form::input($type, $name, $value, ['class' => 'form-control', 'placeholder' => $placeholder, 'required' => ""]) !!}
            @else
